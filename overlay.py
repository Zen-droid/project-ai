...
def draw_bounding_box(img, face_bounding_box, temps):
    top, right, bottom, left = face_bounding_box
    w = right-left
    h = bottom-top

    img = cv2.putText(img, "Temperature = {}".format(temps), (left, bottom+20), cv2.FONT_HERSHEY_PLAIN, 0.8, (255,255,255), 1)
...
def draw_attendance_status(img, small_bounding_box, accepted):
    ...
    img = cv2.putText(img, text, (left, top+int(h/2)), cv2.FONT_HERSHEY_COMPLEX, 1, (255,255,255))


def draw_overlay(img, temps):
    curr_time = attendance_window.get_curr_time()
    face_bounding_box = face_recognition.face_locations(img)

    if face_bounding_box:
        img = draw_bounding_box(img, face_bounding_box[0], temps)
